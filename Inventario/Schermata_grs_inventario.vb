Public Class Schermata_grs_inventario
    Public fp As Form
    ...
    Private Sub DvgCorpo_KeyDown(sender As Object, e As KeyEventArgs) Handles DgvCorpo.KeyDown
        ...
        ElseIf e.KeyCode = Keys.Delete Then
            If DgvCorpo.CurrentRow IsNot Nothing Then
                ...
                Try
                    conn.Open()
                    If Service.CancRigo(TabelleDatabase.tb_articoli, DgvCorpo.CurrentRow.DataBoundItem.row("id"), conn, True) Then
    ...
    Private Sub Modi(Optional rowAppo As DataRow = Nothing)
        If CheckFormAperto(Schermata_grs_inventario_Modi, My.Application.OpenForms) = False Then
            Schermata_grs_inventario_Modi.fp = Me

            If rowAppo IsNot Nothing Then
                Schermata_grs_inventario_Modi.rowCorpo = rowAppo
            Else
                Schermata_grs_inventario_Modi.rowCorpo = dtCorpo.NewRow
                Schermata_grs_inventario_Modi.rowCorpo("cancellato") = False
            End If

            Schermata_grs_inventario_Modi.Show()
        Else
    ...
    Private Function Carica() As Boolean
        Try
            ...
            If conn.State = ConnectionState.Open Then
                dtCorpo = New Query().CaricaInventario(conn)
                DgvCorpo.DataSource = dtCorpo
